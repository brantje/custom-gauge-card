...
customElements.define('action-handler-custom-gauge', ActionHandler);

const getActionHandler = (): ActionHandler => {
  const body = document.body;
  if (body.querySelector('action-handler-custom-gauge')) {
    return body.querySelector('action-handler-custom-gauge') as ActionHandler;
  }

  const actionhandler = document.createElement('action-handler-custom-gauge');
...
export const actionHandler = directive(
  class extends Directive {
    update(part: AttributePart, [options]: DirectiveParameters<this>): symbol {
